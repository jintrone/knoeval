...
public class MyGraphMLExporter<V, E>
{
    ...
    private EdgeWeightProvider<E> edgeWeightProvider;

    private List<AttributeProvider> edgeProviders = new ArrayList<>();
    private List<AttributeProvider>  vertexProviders = new ArrayList<>();
    ...
    public MyGraphMLExporter(
            ...
            EdgeWeightProvider<E> edgeWeightProvider,
            AttributeProvider... providers)
    {
        this.vertexIDProvider = vertexIDProvider;
        this.vertexLabelProvider = vertexLabelProvider;
        this.edgeIDProvider = edgeIDProvider==null?new IntegerEdgeNameProvider<E>():edgeIDProvider;
        this.edgeLabelProvider = edgeLabelProvider;
        this.edgeWeightProvider = edgeWeightProvider;

        for (AttributeProvider p:providers) {
            if (p.getType() == AttributeType.EDGE) {
                edgeProviders.add(p);
            } else {
                vertexProviders.add(p);
            }
        }
    }
            ...
            throws SAXException, TransformerConfigurationException
    {
        ...
        for (AttributeProvider p:vertexProviders) {
            // <key> for vertex label attribute
            attr.clear();
            attr.addAttribute("", "", "id", "CDATA", p.getName());
            attr.addAttribute("", "", "for", "CDATA", "node");
            attr.addAttribute("", "", "attr.name", "CDATA", p.getName());
            attr.addAttribute("", "", "attr.type", "CDATA", p.getDataType());
            handler.startElement("", "", "key", attr);
            handler.endElement("", "", "key");
        }
        ...
        if (edgeWeightProvider != null) {
            attr.clear();
            attr.addAttribute("", "", "id", "CDATA", "edge_weight");
            attr.addAttribute("", "", "for", "CDATA", "edge");
            attr.addAttribute("", "", "attr.name", "CDATA", "Edge Weight");
        ...
        for (AttributeProvider p:edgeProviders) {
            // <key> for vertex label attribute
            attr.clear();
            attr.addAttribute("", "", "id", "CDATA", p.getName());
            attr.addAttribute("", "", "for", "CDATA", "edge");
        ...
        for (E e : g.edgeSet()) {
            ...
            if (edgeWeightProvider != null) {
                ...
                // Content for <data>
                Float f = edgeWeightProvider.getEdgeWeight(e);
                String edgeWeight = String.format("%.3f",f.doubleValue());
    ...
    private void handleAttribute(TransformerHandler txHandler, AttributesImpl attrImpl, AttributeProvider attrProvider,  V elt) throws SAXException {
        attrImpl.clear();
        attrImpl.addAttribute("", "", "key", "CDATA", attrProvider.getName());
        txHandler.startElement("", "", "data", attrImpl);

        // Content for <data>
        //Float f = edgeWeightProvider.getEdgeWeight(e);
        //String edgeWeight = String.format("%.3f",f.doubleValue());
        String value = attrProvider.getValue(elt);
        txHandler.characters(
                value.toCharArray(),
                0,
                value.length());
        txHandler.endElement("", "", "data");
